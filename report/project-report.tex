...
Only a2a has more changing builds than non-changing builds. But then, most of these changes are really small, as shown in Fig. \ref{a2aHist}. In other metrics most changes are quite small as well, with only few big architectural changes (figure \ref{cvgHist} and \ref{numEdgesHist}). This seems usual, because architecture changes only sometimes in a big manner.
...
We approached this result through an easily expandable framework to recover the architecture from the builds using one extractor, two reconstructors and three metric sets. In total, $8$ metrics were tested for correlation in $6$ different scenarios including the previous and following builds. 
